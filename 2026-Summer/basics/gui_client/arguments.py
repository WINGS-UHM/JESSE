#!/usr/bin/env python3
"""
Command-line settings for the modular GUI client.

`argparse` lets the same code run with different values without editing files.

Example run:

    python3 -m gui_client_student --xapp-url http://192.168.50.247:8080

Concrete defaults from `constants.py`:
...
def parse_args() -> argparse.Namespace:
    """
    Build and validate command-line arguments.

    Add-argument syntax:

        parser.add_argument("--name", type=float, default=10.0, help="text")
    ...
    parser.add_argument("--video-url", default=None, help="Override video stream URL")
    ...
    parser.add_argument("--video-timeout", type=float, default=DEFAULT_GUI_CONFIG["video_timeout"], help="Video HTTP timeout seconds")
    ...
    parser.add_argument("--no-calibrate-pedals", dest="calibrate_pedals", action="store_false", help="Use fixed pedal idle convention")
    ...
    parser.add_argument("--arm-default-button", type=int, default=DEFAULT_GUI_CONFIG["arm_default_button"], help="Button that sends the default arm pose; -1 disables")
    ...
    parser.add_argument("--print-commands", action="store_true", help="Print axes and JSON payloads while sending")

    args = parser.parse_args()

    # Validation examples.  `parser.error(...)` prints a message and exits.
    if args.rate_hz <= 0:
