#!/usr/bin/env python3
"""
Dashboard drawing code for pygame.
...
Example:
    ...
    (18, 20, 24) is the dark background color.
"""
...
class DashboardRenderer:
    def draw_video_frame(self) -> None:
        """
        Decode the newest video frame and redraw the dashboard.

        ## TODO
        Set `frame` and `frame_id` inside the lock:

            frame = self.video_frame
            frame_id = self.video_frame_id

        If `frame is not None` and `frame_id != self.video_drawn_id`, set:

            self.video_surface = self.pygame.image.load(io.BytesIO(frame)).convert()
            self.video_drawn_id = frame_id

        After the frame check, call:
    ...
    def draw_dashboard(self) -> None:
        """
        Draw the full dashboard layout.

        Concrete layout values:

            margin = 14
            status panel width min = 280
            status panel width max = 340
            bottom panel height = 170
        ...
        Set `video_content_rect` to:

            self.pygame.Rect(video_rect.x + 8, video_rect.y + 44, video_rect.width - 16, video_rect.height - 52)
    ...
    def draw_status_panel(self, window: Any, rect: Any) -> None:
        """
        Draw status rows and event log.
    ...
    def draw_bottom_panel(self, window: Any, rect: Any) -> None:
        """
        Draw steering/throttle/brake bars and arm servo values.
        ...
        Draw these bars:
            ...
            steering bar value = float(command.get("steering", 0.0)), range -1.0 to 1.0
            throttle bar value = float(command.get("throttle", 0.0)), range 0.0 to 1.0
            brake bar value = float(command.get("brake", 0.0)), range 0.0 to 1.0

        Set:

            arm_x = x + 340

        For each arm row, set:
        ...
        for offset, servo_id in enumerate(ARM_SERVO_IDS):
            ...
            row = offset // 2
            col = offset % 2
            self.draw_text(window, f"id{servo_id:<2} {curr:>4}  {lower}-{upper}", (arm_x + col * 210, y + 34 + row * 30), (190, 205, 215), self.small_font)
