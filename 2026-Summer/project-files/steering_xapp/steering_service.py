...
class RobotTarget:
    ...
    depth_raw_path: str = "/depth/raw"
    depth_topic: str = "/depth_cam/depth0/image_raw"
...
class SteeringCommandService:
    ...
    def apply_config(self, config: Dict[str, Any]) -> None:
        ...
        self.robot = RobotTarget(
            ...
            depth_raw_path=str(robot.get("depthRawPath", "/depth/raw")),
            depth_topic=str(robot.get("depthTopic", "/depth_cam/depth0/image_raw")),
    ...
    def arm_snapshot(self) -> Dict[str, Any]:
        with self.lock:
            return {
                "servos": {
                    str(servo_id): {
                        "curr": self.arm_state.positions.get(servo_id),
                        "min": limits[0],
                        "max": limits[1],
                    }
                    for servo_id, limits in DEFAULT_ARM_LIMITS.items()
                },
                "state": {
    ...
    def video_snapshot(self, query: str = "") -> Tuple[int, bytes, str]:
    ...
    def depth_snapshot(self, query: str = "") -> Tuple[int, bytes, str]:
        try:
            raw = self.fetch_raw_depth(query=query)
            image = self.render_depth_image(raw)
            buffer = io.BytesIO()
            image.save(buffer, format="JPEG", quality=80, optimize=False)
            return 200, buffer.getvalue(), "image/jpeg"
        except Exception as exc:
            if self.logger is not None:
                self.logger.error(f"xApp depth processing failed: {exc}")
            body = json.dumps({"error": "xApp depth processing failed", "detail": str(exc)}, sort_keys=True).encode("utf-8")
            return 502, body, "application/json"

    def fetch_raw_depth(self, query: str = "") -> Dict[str, Any]:
        if not self.robot.base_url:
            raise ValueError("robot baseUrl is not configured")
        path = self.robot.depth_raw_path
        url = f"{self.robot.base_url}{path if path.startswith('/') else '/' + path}"
        if query:
            url = f"{url}?{query}"
        elif self.robot.depth_topic:
            url = f"{url}?{urlencode({'topic': self.robot.depth_topic}, safe='/')}"
        response = requests.get(url, timeout=max(self.robot.timeout_seconds, 4.0))
        response.raise_for_status()
        return response.json()

    def render_depth_image(self, raw: Dict[str, Any]) -> Image.Image:
        width = int(raw.get("width", 0))
        height = int(raw.get("height", 0))
        step = int(raw.get("step", width * 2))
        encoding = str(raw.get("encoding", "")).lower()
        if width <= 0 or height <= 0:
            raise ValueError("raw depth frame is missing width/height")
        if encoding not in ("mono16", "16uc1", "16sc1"):
            raise ValueError(f"unsupported depth encoding {encoding!r}")

        data = base64.b64decode(str(raw.get("data", "")))
        dtype = ">u2" if int(raw.get("is_bigendian", 0)) else "<u2"
        row_bytes = width * 2
        rows = []
        for row in range(height):
            start = row * step
            rows.append(np.frombuffer(data[start: start + row_bytes], dtype=dtype, count=width))
        ...
        normalized = np.clip((depth - low) * 255.0 / (high - low), 0, 255).astype(np.uint8)
        grayscale = Image.fromarray(normalized, mode="L")
        grayscale = ImageOps.autocontrast(grayscale)
        return ImageOps.colorize(grayscale, black="#2b0a3d", white="#ffd43b", mid="#3465d9").convert("RGB")
